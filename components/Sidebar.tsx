...
export default function Sidebar({ portfolios = [], activePortfolioId }: SidebarProps) {
  ...
  // Lire l'id actif depuis l'URL côté client
  const [currentId, setCurrentId] = useState(activePortfolioId ?? "");
  useEffect(() => {
    if (typeof window !== "undefined") {
      const urlId = new URLSearchParams(window.location.search).get("id");
      setCurrentId(urlId || activePortfolioId || "");
    }
  }, [pathname, activePortfolioId]);

  const activePf = portfolios.find(p => p.id === currentId) ?? portfolios[0];
  ...
  return (
    <>
      <style>{`
        ...
        .sb-dropdown{background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.07);border-radius:6px;padding:10px 13px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:8px;transition:background 0.2s;position:relative}
        .sb-dropdown:hover{background:rgba(255,255,255,.08)}
        .sb-dropdown-text{font-size:11px;color:rgba(255,255,255,.55);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0;font-weight:300}
        ...
        .sb-dropdown-item{display:flex;align-items:center;gap:8px;width:100%;text-align:left;padding:10px 13px;font-size:11px;color:rgba(255,255,255,.45);background:none;border:none;cursor:pointer;font-family:'Inter',sans-serif;font-weight:300;transition:all 0.15s}
        .sb-dropdown-item:hover{background:rgba(255,255,255,.05);color:rgba(255,255,255,.8)}
        .sb-dropdown-item.cur{color:white;background:rgba(255,255,255,.07)}
        ...
        .sb-edit-btn:hover{background:rgba(255,255,255,.12);color:rgba(255,255,255,.7)}
        ...
        .modal-toggle-btn{flex:1;padding:8px;font-size:10px;font-weight:500;border:none;border-radius:4px;cursor:pointer;font-family:'Inter',sans-serif;letter-spacing:.06em;transition:all 0.15s}
        ...
        .modal-btn{flex:1;padding:12px;font-size:10px;font-weight:500;letter-spacing:.12em;border:none;border-radius:8px;cursor:pointer;font-family:'Inter',sans-serif;transition:all 0.2s}
      ...
      <aside className="sb">
        <div className="sb-header">
          ...
          {portfolios.length === 0 ? (
            <Link href="/dashboard/entry" className="sb-btn-entry">+ RENSEIGNER MON PORTEFEUILLE</Link>
          ) : (
            <div className="sb-dropdown" onClick={()=>setDropOpen(!dropOpen)}>
              ...
              {dropOpen && (
                <div className="sb-dropdown-menu" onClick={e=>e.stopPropagation()}>
                  {portfolios.map(pf => (
                    <div key={pf.id} className={`sb-dropdown-item${pf.id===currentId?" cur":""}`}
                      style={{cursor:"default"}}>
                      <div style={{display:"flex",alignItems:"center",gap:7,flex:1,minWidth:0,cursor:"pointer"}}
                  ...
                  <div className="sb-dropdown-item" style={{borderTop:"1px solid rgba(255,255,255,.06)"}}>
                    <button style={{background:"none",border:"none",color:"rgba(255,255,255,.3)",fontSize:10,cursor:"pointer",fontFamily:"'Inter',sans-serif",padding:0,letterSpacing:".04em"}}
