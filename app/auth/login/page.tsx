...
export default function LoginPage() {
  ...
  return (
    <AuthLayout title1={t1} title2={t2}
      ...
      <h2 style={{...stg(5),fontSize:28,fontWeight:500,color:C.navyText,letterSpacing:"-.02em",marginBottom:8}}>Connexion</h2>
      <p style={{...stg(5),fontSize:13,fontWeight:400,color:"rgba(5,11,20,0.4)",marginBottom:32}}>
        Pas encore de compte ? <span onClick={()=>router.push("/auth/register")} style={{color:C.sapphire,cursor:"pointer",fontWeight:500}}>S'inscrire gratuitement</span>
      </p>

      <form onSubmit={handleSubmit}>
        <div style={stg(6)}>
          <label style={{display:"block",fontSize:10,fontWeight:500,letterSpacing:".1em",textTransform:"uppercase",color:"rgba(5,11,20,0.36)",marginBottom:8}}>Adresse email</label>
          <input type="email" required value={email} onChange={e=>setEmail(e.target.value)}
            onFocus={()=>setFocusField("email")} onBlur={()=>setFocusField(null)}
            placeholder="vous@exemple.fr" style={{...inputStyle("email"),marginBottom:20}}/>
        </div>

        <div style={stg(7)}>
          <label style={{display:"block",fontSize:10,fontWeight:500,letterSpacing:".1em",textTransform:"uppercase",color:"rgba(5,11,20,0.36)",marginBottom:8}}>Mot de passe</label>
          <input type="password" required value={password} onChange={e=>setPassword(e.target.value)}
            onFocus={()=>setFocusField("password")} onBlur={()=>setFocusField(null)}
            placeholder="••••••••" style={{...inputStyle("password"),marginBottom:28}}/>
        </div>
        ...
        <div style={stg(8)}>
          <button type="submit" disabled={loading}
            ...
            <span style={{position:"relative",zIndex:1}}>{loading?"Connexion en cours...":"Se connecter"}</span>
          </button>
        </div>

        <div style={{...stg(9),display:"flex",alignItems:"center",gap:16,margin:"24px 0"}}>
