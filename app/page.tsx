...
input[type=range] {
  ...
  background:rgba(26,26,26,0.18); outline:none; cursor:pointer;
}
input[type=range]::-webkit-slider-thumb {
  -webkit-appearance:none; width:12px; height:12px; border-radius:50%;
  background:#1A1A1A; border:2px solid #F9F8F6;
  box-shadow:0 0 0 1px rgba(26,26,26,0.2);
}
`;

// ══════════════════════════════════════════════════════════════
// HERO — Canvas plein écran + titre immersif
// ══════════════════════════════════════════════════════════════
function HeroSection({
  ...
  function drawFrame(zPts: number[], bPts: number[], canvas: HTMLCanvasElement) {
    ...
    const n = zPts.length;
    // Pas de padding gauche — la courbe part du bord
    // Pas de labels d'axe — épuré total
    const PAD_L = 0;
    const PAD_R = 0;
    const PAD_T = H * 0.08;   // 8% de marge en haut
    const PAD_B = H * 0.12;   // 12% de marge en bas

    const drawW = W - PAD_L - PAD_R;
    const drawH = H - PAD_T - PAD_B;

    // Échelle — la courbe Zero part bas gauche et monte haut droite
    const maxV = zPts[n-1] * 1.04;
    const minV = zPts[0]   * 0.88;
    const rng  = maxV - minV;

    const sx = (i: number) => PAD_L + (i/(n-1)) * drawW;
    const sy = (v: number) => PAD_T + drawH - ((v - minV)/rng) * drawH;

    // ── Zone de profit entre les deux courbes ─────────────
    ct.beginPath();
    ct.moveTo(sx(0), sy(zPts[0]));
    for (let i=1; i<n; i++) {
      const cpx = sx(i-0.5);
      ct.quadraticCurveTo(cpx, sy(zPts[i-1]), sx(i), sy(zPts[i]));
    }
    for (let i=n-1; i>=0; i--) {
      if (i===n-1) ct.lineTo(sx(i), sy(bPts[i]));
      else ct.quadraticCurveTo(sx(i+0.5), sy(bPts[i+1]), sx(i), sy(bPts[i]));
    }
    ct.closePath();
    const gZone = ct.createLinearGradient(sx(0), 0, sx(n-1), 0);
    gZone.addColorStop(0,    "rgba(134,239,172,0.00)");
    gZone.addColorStop(0.3,  "rgba(134,239,172,0.04)");
    gZone.addColorStop(0.75, "rgba(134,239,172,0.08)");
    gZone.addColorStop(1,    "rgba(134,239,172,0.14)");
    ct.fillStyle = gZone;
    ct.fill();

    // ── Courbe Banque — tiretée, très discrète ────────────
    ct.beginPath();
    ct.setLineDash([3,7]);
    ct.moveTo(sx(0), sy(bPts[0]));
    for (let i=1; i<n; i++) {
      ct.quadraticCurveTo(sx(i-0.5), sy(bPts[i-1]), sx(i), sy(bPts[i]));
    }
    ct.strokeStyle = "rgba(26,26,26,0.20)";
    ct.lineWidth   = 1.2;
    ct.stroke();
    ct.setLineDash([]);

    // ── Courbe Zero CGP — principale, profonde ────────────
    ct.beginPath();
    ct.moveTo(sx(0), sy(zPts[0]));
    for (let i=1; i<n; i++) {
      ct.quadraticCurveTo(sx(i-0.5), sy(zPts[i-1]), sx(i), sy(zPts[i]));
    }
    ...
    gLine.addColorStop(0.8, "rgba(10,22,40,0.82)");
    ...
    // ── Dot final Zero CGP ────────────────────────────────
    const ex  = sx(n-1);
    const ezy = sy(zPts[n-1]);
    const eby = sy(bPts[n-1]);

    // Halo
    ct.beginPath();
    ct.arc(ex, ezy, 14, 0, Math.PI*2);
    ct.fillStyle = "rgba(10,22,40,0.06)";
    ct.fill();
    ct.beginPath();
    ct.arc(ex, ezy, 7, 0, Math.PI*2);
    ct.fillStyle = "rgba(10,22,40,0.12)";
    ct.fill();
    // Point
    ct.beginPath();
    ct.arc(ex, ezy, 4, 0, Math.PI*2);
    ct.fillStyle = "#0A1628";
    ct.fill();
    ct.beginPath();
    ct.arc(ex, ezy, 2, 0, Math.PI*2);
    ct.fillStyle = "#F9F8F6";
    ct.fill();

    // Dot banque
    ct.beginPath();
    ct.arc(ex, eby, 3, 0, Math.PI*2);
    ...
    ct.moveTo(ex, eby);
    ct.lineTo(ex, ezy);
    ct.strokeStyle = "rgba(134,239,172,0.5)";
    ct.lineWidth   = 1;
    ct.stroke();
    ct.setLineDash([]);
    ...
    if (!curveReady) setCurveReady(true);
  }

  function morphCurves(
    from: {z:number[];b:number[]},
    to:   {z:number[];b:number[]}
  ) {
  ...
  function updateChart(animate = true) {
    ...
    if (!prevZRef.current || !animate) {
      ...
      prevBRef.current = [...newB];
      drawFrame(newZ, newB, canvas);
    } else {
      morphCurves(
        {z: prevZRef.current, b: prevBRef.current},
        {z: newZ, b: newB}
      );
    }
  }

  useEffect(() => { updateChart(false); }, []);
  useEffect(() => { updateChart(true);  }, [capital, years]);
  useEffect(() => {
    function onResize() {
  ...
  // Stagger du contenu après montage
  const wrapV = {
    hidden:  {},
  ...
  return (
    <section className="hero-grain" style={{
      ...
      {/* z-index:2 — derrière le texte z:4/5, devant le fond z:0 */}
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", gap: 2 }}>
          {["Accueil","Divergence","Comment ça fonctionne","Simulation"].map((label,i) => (
      ...
      {/* ── Titre immersif — z:4, passe devant la courbe ─────── */}
      <div style={{
        position: "absolute",
        top: "50%", left: "52px",
        transform: "translateY(-56%)",
        zIndex: 4, maxWidth: "55vw",
      }}>
        <motion.div
          variants={wrapV}
          initial="hidden"
        ...
        >
          ...
          {/* Titre principal — massif, opacity 0.95 */}
          ...
          <motion.p variants={itemV} style={{
            ...
            color: "rgba(10,22,40,0.44)", lineHeight: 1.75,
            marginBottom: 28,
          }}>
      ...
      {/* ── Montants finaux — droite, immenses ────────────────── */}
      {curveReady && (
        <motion.div
          initial={{ opacity:0 }}
          animate={{ opacity:1 }}
        ...
        >
          ...
          {/* Banque — valeur secondaire, atténuée */}
          <div style={{
            fontFamily: "'Inter',sans-serif",
            fontSize: 9, fontWeight: 500, letterSpacing: ".16em",
            color: "rgba(10,22,40,0.24)", textTransform: "uppercase",
            marginBottom: 4,
          }}>Banque privée</div>
          <div style={{
            fontFamily: "'Cormorant Garant',serif",
            fontSize: "clamp(22px,2.8vw,38px)",
            fontWeight: 300, color: "rgba(10,22,40,0.38)",
            lineHeight: 1, letterSpacing: "-.02em",
            marginBottom: 20,
          }}>{feur(bankFinal)}</div>
          ...
          {/* Gain — pill vert */}
          <div style={{
            background: NAVY, borderRadius: 9,
            padding: "10px 16px", display: "inline-block",
          }}>
            <div style={{
              fontFamily: "'Inter',sans-serif",
              fontSize: 8, fontWeight: 500, letterSpacing: ".14em",
              color: "rgba(255,255,255,0.32)", textTransform: "uppercase",
              marginBottom: 3,
            }}>Économies</div>
            <div style={{
              fontFamily: "'Cormorant Garant',serif",
              fontSize: 24, fontWeight: 300, color: "#4ADE80",
      ...
      {/* ── Contrôles minimalistes — barre du bas ────────────── */}
      <div style={{
        ...
        <span style={{
          ...
          color: "rgba(10,22,40,0.32)", textTransform: "uppercase", whiteSpace: "nowrap",
        }}>Capital</span>
        <input
        ...
        <div style={{ display:"flex", gap:4 }}>
          {[10,15,20,25].map(y => (
            <button key={y} onClick={() => onYearsChange(y)} style={{
              ...
              color: y===years ? "#F9F8F6" : "rgba(10,22,40,0.34)",
